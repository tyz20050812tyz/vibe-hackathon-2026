...
export async function listSavedResources(supabase: SupabaseClient): Promise<SavedResource[]> {
...
export async function saveResource(supabase: SupabaseClient, input: CreateSavedResourceInput): Promise<SavedResource> {
  const id = await userId(supabase);
  ...
  const saved = await listSavedResources(supabase);
  const resource = saved.find((item) => item.resource.id === input.resourceId);
  if (!resource) throw internal("收藏已保存，但无法读取资源详情。");
  return resource;
}

export async function removeSavedResource(supabase: SupabaseClient, resourceId: string) {
  const id = await userId(supabase);
