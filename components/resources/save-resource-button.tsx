...
export function SaveResourceButton({ resourceId }: { resourceId: string }) {
  ...
  const save = async () => {
    ...
    try {
      ...
      const response = await fetch("/api/saved-resources", { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${data.session.access_token}` }, body: JSON.stringify({ resourceId, note: note.trim() || undefined }) });
