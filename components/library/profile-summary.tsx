...
export function ProfileSummary({ profile, enabled, onSave }: {
  ...
  const initialName = profile?.displayName ?? "阅读者";
  ...
  const displayName = profile?.displayName ?? "阅读者";
  ...
  return <section className="grid gap-5 border-b border-[#254a42]/20 pb-7 sm:grid-cols-[auto_1fr_auto] sm:items-center">
    ...
    {enabled ? <div className="flex items-center gap-2"><button type="button" onClick={() => { if (editing) { void save(); } else { setName(profile?.displayName ?? "阅读者"); setEditing(true); } }} disabled={saving} className="inline-flex h-10 items-center gap-2 border border-[#254a42] px-3 text-sm text-[#254a42] hover:bg-[#254a42] hover:text-[#fff8e9] disabled:opacity-60">{editing ? <Check className="size-4" /> : <PencilLine className="size-4" />}{saving ? "正在保存" : editing ? "保存" : "编辑资料"}</button>{editing ? <button type="button" onClick={() => { setEditing(false); setMessage(""); setName(profile?.displayName ?? "阅读者"); }} className="h-10 px-2 text-sm text-[#52625d] hover:underline">取消</button> : null}</div> : <div className="inline-flex items-center gap-2 text-sm text-[#52625d]"><UserRound className="size-4" />档案准备中</div>}
  </section>;
}
