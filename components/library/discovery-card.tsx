...
export function DiscoveryCard({ items, sourceTitle, loading, error, onRetry }: {
  ...
  return <section className="border border-[#254a42] bg-[#244f46] p-5 text-[#fff8e9] sm:p-6">
    <div className="flex items-start justify-between gap-5"><div><p className="flex items-center gap-2 text-sm text-[#e4e7d4]"><Compass className="size-4" />下一条阅读偏离</p><h2 className="mt-3 font-serif text-3xl">带我偏离一点</h2><button type="button" onClick={onRetry} disabled={loading} className="mt-4 inline-flex h-9 items-center gap-2 border border-[#fff8e9]/45 px-3 text-sm text-[#fff8e9] hover:bg-[#fff8e9] hover:text-[#244f46] disabled:opacity-60"><Compass className="size-4" />{loading ? "正在寻找" : "开始偏离"}</button></div><Sparkles className="size-6 text-[#d2a85a]" aria-hidden="true" /></div>
